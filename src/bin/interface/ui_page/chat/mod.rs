    ...
    W: WriteExt + Unpin,
{
    ...
    write!(stdout, "{}{}{:?}{}{}{:?}\n", cursor::Goto(1, 17), style::Bold, color::Rgb(227, 227, 41), "-".repeat(80), style::Reset, color::Reset)
        .map_err(|e| UserError::WriteError(e))?;
    stdout.flush()
        .map_err(|e| UserError::WriteError(e))?;

    // Write prompt
    write!(stdout, "{}{:?}{}", cursor::Goto(1, 18), color::Rgb(250, 250, 237), ">>>")
        .map_err(|e| UserError::WriteError(e))?;
    stdout.flush()
        .map_err(|e| UserError::WriteError(e))?;
    ...
    let mut msg_queue = VecDeque::with_capacity(8);
    ...
    loop {
        ...
        for msg in msg_queue.iter().rev() {
            write!(guard, "{}{}{:?}{}", cursor::Goto(1, row), clear::CurrentLine, color::Rgb(215, 247, 241), msg)
                .map_err(|e| UserError::WriteError(e))?;
            guard.flush()
                .map_err(|e| UserError::WriteError(e))?;
            row -= 1;
