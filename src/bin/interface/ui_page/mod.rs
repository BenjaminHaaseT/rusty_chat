...
impl UIPage {
            ...
            UIPage::LobbyPage {username, lobby_state} => {
                ...
                match response {
                    Response::ExitLobby => {
                        ...
                        // Reset cursor
                        write!(stdout, "{}", cursor::Show).map_err(|e| UserError::WriteError(e))?;
                        stdout.flush().map_err(|e| UserError::WriteError(e))?;
    ...
    where
        ...
        W: WriteExt + Unpin,
    {
        // For writing to stdout in with consistent style
        let mut stdout = stdout().into_raw_mode().map_err(|e| UserError::RawOutput(e))?;
            ...
            UIPage::LobbyPage {username, lobby_state} => {
                ...
                match users_request.as_str() {
                    ...
                    "c" => {
                        ...
                        write!(stdout, "{}{}", cursor::Goto(1, prompt_offset), clear::AfterCursor)
                            .map_err(|e| UserError::WriteError(e))?;
                        stdout.flush().map_err(|e| UserError::WriteError(e))?;
                        ...
                        let chatroom_name = loop {
                            ...
                            // Read buffer from client input
                            let buf = read_line_from_client_input(&mut stdout, prompt_offset + 1, 16)?;
                            let buf = buf.trim().to_owned();
